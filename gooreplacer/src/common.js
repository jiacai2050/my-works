...
// To avoid exceeding the per-item limit, we store rules in up to 9 separate keys, each holding up to 7KB (7000 characters).
// This provides a safety margin below the 8KB limit and helps prevent quota errors.
const MAX_KEYS = 9; // Use up to 9 keys for storing rules (rules, rules2, ..., rules5, ...)
const CHUNK_SIZE = 7000; // Each key stores up to 7KB of data, leaving a margin below the 8KB limit.
