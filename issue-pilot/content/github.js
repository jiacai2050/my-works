...
const IssuePilotGitHub = {
  ...
  getBody() {
    const el = document.querySelector('.js-comment-body, .comment-body, [data-testid="issue-body"]');
    if (!el) return '';
  ...
  getCommentBoxValue() {
    const textarea = document.querySelector('textarea#new_comment_field, textarea[name="comment[body]"]');
  ...
  getDiffContext(commentBox) {
    ...
    const diffTable = commentBox.closest('.diff-table, .js-diff-table, [data-diff-anchor]');
    if (!diffTable) return null;
    // Get the code lines near the comment
    const lines = diffTable.querySelectorAll('.blob-code-inner');
    const codeLines = Array.from(lines).slice(-10).map(el => el.textContent).join('\n');
    const fileName = diffTable.closest('[data-tagsearch-path]')?.dataset.tagsearchPath
